@extends('layouts.app')
@section('title', Auth::user()->organisation->name)
@section('content')
@section('css')
...
<div class="well firstrow list">
  <div class="row">
    {{ Form::open(array('route' => 'ip.dashboard', 'class' => 'form-search pull-left', 'id' => 'samplelist')) }}
            {{ csrf_field() }}
   {{ Form::text('from', $request->from, ['class' => 'input-field filter-date', 'id' => 'from', 'placeholder' => 'From']) }}
   {{ Form::text('to', $request->to, ['class' => 'input-field filter-date', 'id' => 'to', 'placeholder' => 'To']) }}
    ...
    <table id="listtable" class="table table-striped table-bordered">
      ...
      <tr>
        ...
        @foreach($testtypes as $testtype)
            <td>
              <?php 
                $test_label = generateSlug($testtype, '_');
                echo $sample->$test_label;
              ?></td>
          @endforeach
        <td>{{$sample->total}}</td>
      </tr>
