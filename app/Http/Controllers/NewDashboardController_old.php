...
class NewDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        INNER JOIN (select usr.id as userid, usr.staff_id, usr.name as username,stf.designation from users usr 
        INNER JOIN staff stf ON usr.id = stf.user_id
        where stf.designation = 1) as us ON us.staff_id = cl.staffid
        ...
        $hubs_not_tracking = "SELECT HubName,District,region FROM (
            ...
            SELECT hubid FROM
            (select f.parentid as hubid, COUNT(distinct cl.facilityid) AS Nvisits,count(distinct f.districtid) as district from checklogin cl
            INNER JOIN (select usr.id as userid, usr.name as username, usr.staff_id, stf.designation from users usr 
            INNER JOIN staff stf ON usr.id = stf.user_id
            where stf.designation = 1) as us ON us.staff_id = cl.staffid
            INNER JOIN facility f ON f.id = cl.facilityid
             " . $where_clause . "
