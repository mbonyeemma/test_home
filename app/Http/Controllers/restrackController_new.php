...
class restrackController extends Controller
{
        ...
        try {
            \DB::transaction(function () use ($request, $ret_arr) {
                ...
                foreach ($request['barcodes'] as $barcode) {
                    ...
                    if ($package->type == 2) {
                                  // dd($package);
                                  $packages = Package::where('parent_id', '=', $package->id)->get();
                                  foreach ($packages as $package_minor) {
                                      // dd($package_minor);
                                      
                                      // $child_package = Package::where('barcode', '=', $barcode)->firstOrFail()
                                      $event =  PackageMovementEvent::where('id', '=', $package_minor->latest_event_id)->firstOrFail();
                                      // $event->package_id = $package_minor->id;
                                      // $event->source = $post_data['facilityid'];
                                      // $event->destination =  $post_data['final_destination'];
                                      // $event->longitude =  0;
                                      // $event->latitude =  0;
                                      ...
                                      // $event->category_id = $package_minor->test_type;
                                      $event->location = $request['facilityid'];
                                      // $event->created_by = $post_data['user_id'];
                                      // \Log::info($event);
                                      $event->save();
                                  }
    ...
    private function setParentForChildrenPackages($children_package_barcodes, $package_id, $event_id, $post_data)
    {
        foreach ($children_package_barcodes as $barcode) {
            ...
            $event->package_id = $package_id;
            $event->source = $post_data['facilityid'];
            $event->destination =  $child_package->final_destination;
            $event->longitude =  $post_data['longitude'];
            $event->latitude =  $post_data['latitude'];
            $event->place_name =  $post_data['place_name'];
            $event->status = 1;
            $event->category_id = $child_package->test_type;
            $event->location = $post_data['facilityid'];
            $event->created_by = $post_data['user_id'];
            // \Log::info($event);
            $event->save();
            //WEWEWEWEW
            // $event = $this->createEvent($post_data, $package_id, 1);
            // $child_package->latest_event_id = $event_id;
            $child_package->latest_event_id = $event->id;
            $child_package->save();
        }
    }
    ...
    public function getPackagesBeyondDate($provided_date)
    {
        ...
        $query = "SELECT id, barcode, numberofsamples, created_at FROM package WHERE delivered_on IS NULL AND created_at > '" . $provided_date . "'";
    ...
    public function getPackages($categrory, $cat_id = 0)
    {
        ...
        if ($categrory == 'single') {
            ...
            $query = "SELECT p.barcode,sf.name as source_facility,fd.name as final_destination, ef.name as last_location, p.latest_event_id from packagemovement_events pme
                        ...
                        INNER JOIN facility sf ON(p.facilityid = sf.id)
                        INNER JOIN facility fd ON(p.final_destination = fd.id)
                        WHERE (pme.status < 2 OR pme.status = 5) AND p.parent_id = 0 AND pme.created_by = " . $cat_id . " AND  pme.created_at between (CURDATE() - INTERVAL 1 MONTH ) and (CURDATE() + INTERVAL 1 DAY)";
        } else {
