...
ORDER BY f.name";
	...
	function getDailyRoutingForBike($hubid, $bikeid, $thedate){
		...
		INNER JOIN lookuptypevalue lv ON (lv.id = dd.test_type AND lv.lookuptypeid)
...
function getWeekEndDates()
{
	...
	$dotw = $dotw = date('m', $date);
	// $start = ($dotw == 1 /* Monday */) ? $date : strtotime('last Monday', $date);
	// $end = ($dotw == 0 /* Sunday */) ? $date : strtotime('next Sunday', $date);
	$start = date("m/d/Y", strtotime(date("m/d/Y", strtotime(date("m/d/Y"))) . "-1 month"));
	$end = date("m/d/Y", strtotime(date("m/d/Y", strtotime(date("m/d/Y"))) . "0 month"));
	// return array('start' => date('m/d/Y', $start), 'end' => date('m/d/Y', $end));
	return array('start' => date($start), 'end' => date($end));
}
...
function getPublicTAT($picked_at, $recieved_at,$final_destination,$le_source,$le_destination,$status){
	if($final_destination == $le_source && $status == 3){
		if(trim($picked_at) != '' && trim($recieved_at) != ''){
			$from_secs = strtotime(trim($picked_at));
			$to_secs = strtotime(trim($recieved_at));
			$sec_def = $to_secs - $from_secs;
			if($sec_def){
				return time_elapsed($to_secs - $from_secs);
			}else{
				return 0;
...
function time_elapsed($secs){
  ...
  function getCPHLGPS(){
  	return ['latitude'=>0.313435,'longitude'=>32.659619];
  }

  function isPoeOrEocUser($user_id){
  		$user_roles = \DB::select('SELECT * FROM role_user WHERE user_id = '.$user_id);
  		if(count($user_roles) == 1 && ($user_roles[0]->role_id == 13 || $user_roles[0]->role_id == 17)){
  			return true;
  		}else{
  			return false;
  		}
	}
 function generateArrayForEach($arr){
 	$obj_arr = array();
 	$i = 0;
 ...
 function generateMultiKeyValue($arr_obj){
 	//dd($arr);
 	$obj_arr = array();
 	for($i = 0; $i < count($arr_obj); $i++){
 		$obj_arr[$i]['facility_id'] = $arr_obj[$i]->id;
 		$obj_arr[$i]['facility_name'] = $arr_obj[$i]->name;
 		$obj_arr[$i]['hubid'] = $arr_obj[$i]->parentid;
		 $obj_arr[$i]['is_ref_lab_and_hub'] = $arr_obj[$i]->is_ref_lab_and_hub;
 	}
 	return $obj_arr;
 }
